...
class OutputController:
    ...
    def _setup_output_controller(self, output_dir: str):
        """设置输出控制器"""
        ...
        # 分层锁架构：替换单一全局锁为细粒度锁
        self.monitor_locks: Dict[str, threading.Lock] = {}  # 每个监控器的缓冲区锁
        self.csv_locks: Dict[str, threading.Lock] = {}  # 每个CSV文件的写入锁
        ...
        # 缓冲区和批处理相关
        self.events_buffer: Dict[str, deque] = defaultdict(lambda: deque(maxlen=self.buffer_size))
        self.batch_size = 100  # 批处理大小（消费者端使用）

        # 表头输出控制
        self.header_printed: Dict[str, bool] = {}

        self.logger.info("输出控制器初始化完成")

    def _apply_config(self, config: OutputConfig) -> None:
        """应用配置"""
    ...
    def register_monitor(self, monitor_type: str, monitor_instance: BaseMonitor) -> None:
        """注册监控器"""
        with self.buffer_mgmt_lock:
            ...
            self.logger.info(f"注册监控器: {monitor_type}")

    def unregister_monitor(self, monitor_type: str) -> None:
        """注销监控器"""
        with self.buffer_mgmt_lock:
            if monitor_type in self.monitors:
                ...
                self.logger.info(f"注销监控器: {monitor_type}")
    ...
    def _update_output_mode(self) -> None:
        ...
        if old_mode != self.output_mode:
            self.logger.info(f"输出模式切换: {old_mode.name} -> {self.output_mode.name}")

    def _setup_csv_file(self, monitor_type: str) -> None:
        """设置CSV文件"""
        try:
            # 生成文件名
            timestamp = time.strftime('%Y%m%d_%H%M%S')
            filename = f"{monitor_type}_{timestamp}.csv"
            ...
            self.logger.info(f"创建CSV文件: {filepath}")

        except Exception as e:
            self.logger.error(f"创建CSV文件失败 {monitor_type}: {e}")

    def _close_csv_file(self, monitor_type: str) -> None:
        """关闭CSV文件"""
        if monitor_type in self.csv_files:
            try:
                self.csv_files[monitor_type].close()
                self.logger.info(f"关闭CSV文件: {monitor_type}")
            except Exception as e:
                self.logger.error(f"关闭CSV文件失败 {monitor_type}: {e}")
            finally:
                # 清理引用，使用pop方法更简洁
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"输出控制器启动失败: {e}")
            return False

    def handle_event(self, monitor_type: str, event: BaseEvent) -> None:
        """
        ...
        try:
            ...
            self.logger.error(f"处理eBPF事件失败 {monitor_type}: {e}")
    ...
    def _output_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.logger.error(f"输出处理错误: {e}")
                time.sleep(1)

    def _process_buffer(self, monitor_type: str) -> None:
        """处理监控器缓冲区 - 消费者端批处理"""
    ...
    def _process_event_batch(self, monitor_type: str, events: list):
        """批量处理事件 - 减少I/O系统调用次数"""
        try:
            # 批量CSV写入
            ...
            self.logger.error(f"批处理事件失败 {monitor_type}: {e}")

    def _write_csv_batch(self, monitor_type: str, events: list):
        """批量CSV写入 - 使用文件级锁"""
        ...
        with csv_lock:
            ...
            for event in events:
                try:
                    ...
                    self.logger.error(f"CSV写入失败 {monitor_type}: {e}")
            ...
            if len(events) >= 20:
                try:
                    ...
                    self.logger.error(f"CSV刷盘失败 {monitor_type}: {e}")

    def _write_console_batch(self, monitor_type: str, events: list):
        """批量控制台输出 - 使用控制台级锁"""
        with self.console_lock:
            ...
            if not self.header_printed.get(monitor_type, False):
                try:
                    ...
                    self.logger.error(f"控制台表头输出失败: {e}")
            ...
            if csv_lock:
                with csv_lock:
                    try:
                        ...
                        self.logger.error(f"刷新文件失败 {monitor_type}: {e}")
            else:
                # 兜底处理
                try:
                    csv_file.flush()
